...
- **§2.B4 `AppSource__c` populated on 100% of rows** — every row tagged with the surface that originated it (`omens`, `turtleshell-web`, `iris-portal`, `olympus-gpt`, etc.). Resolves §13 D12. Observable: `SELECT AppSource__c, COUNT(Id) FROM LedgerEntry__c WHERE CreatedDate = LAST_N_DAYS:7 GROUP BY AppSource__c` returns no null bucket.
...
| **EOS-3 §13 D17** (forward-only observation 2026-06-10) | iris portal Athena chatbot + Plutus event stream are NOT cluster-specific; iris portal lacks EOS-1/2/3/4 feature-parity with omens/ts-web/ts-ios | **Candidate EOS-5 work item OR own dedicated iris-portal-feature-parity cycle.** Steward direction: deferred until after current prod validation arc closes. Pattern: port the omens/ts-web cluster-picker pattern + wire Athena `apiBase` + wire Plutus event-stream subscriber URL per selected cluster. Not load-bearing unless iris portal is repositioned as a consumer chat surface. |
