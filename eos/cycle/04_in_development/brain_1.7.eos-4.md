...
| **omens iPhone** (consumer Godot/C# surface) | Sign-in against PRODUCTION as `homer@cloudpremise.com` ✅; cluster picker rendered both `eos-4` (initially Provisioning) and `api-int` (Live); **Refresh re-pulled the list correctly after eos-4 went Live** — real-time cosmos-logos discovery validated, not cached. Full play-cyclops-and-submit-feedback round-trip against prod still in flight. | EOS-3 §13 "Fourth cross-cycle re-attestation" |
