...
- `01_planning/` holds THREE follow-on scaffolds opened at EOS-5 close, per Steward direction 2026-07-03: *"Basically anything you want before i accept money needs to be listed in 5.1, 5.2, or 5.3."*
  - **`brain_1.7.eos-5.1.md`** (currently in `04_in_development/`) — Guardians global-launch compliance chain; regulatory + legal architecture (incl. money-transmitter analysis + DAF vs. direct-grant decision). Source spec: `foundation/GUARDIANS-LAUNCH-COMPLIANCE.md`.
  - **`brain_1.7.eos-5.2.md`** — Guest-access lockdown; reconcile EOS agent's empirical `/v1/athena/chat` no-auth finding with backend-agent's api-key-pipeline audit; resolve GAP-A/B/C/D/E from the audit; prove no anonymous fall-through on any revenue-attributing endpoint. See `./eos-5b-triage.md` §5.9 + §5.10 for context.
  - **`brain_1.7.eos-5.3.md`** — Tithe integrity + first-dollar-through operational preconditions. Cause resolution (GAP-28), tithe trigger, Stripe/Apple webhook handlers, idempotency, reconciliation, refund/chargeback reversal, DAF payout architecture (dep on 5.1), public `/tithe` ledger. This cycle closes the gap between EOS-5 READINESS and first-dollar-through-with-integrity. Steward 2026-07-03: *"we are not just selling a good software platform we are selling impact generators."*
