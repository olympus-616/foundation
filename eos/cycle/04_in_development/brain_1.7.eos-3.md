...
**Tally as of 2026-06-10 (fifth update, post-olympus-gpt-substantive-validation): 7 of 9 criteria validated.** §2.4 olympus-gpt now ✅ substantively via FB-00052+53 + perfect two-cluster ledger attribution (LE-103584 int + LE-103578 eos-4) + dual-cluster CloudWatch confirmation. **Four surfaces now formally pass all four cycles against PRODUCTION end-to-end** — omens iPhone + turtleshell-web (post D16 fix) + turtleshell-ios (out of the box) + olympus-gpt (out of the box, hotreload with strict-EOS-4 pending next managed package). Closure-on-brain still requires §2.7 (turtleshell-offgrid) + §2.9 repeatability check. **The EOS-4 §1.1 articulation has been sharpened by the gpt test:** the brain-IS-production claim splits into (1) source code at brain HEAD + (2) prod-hosted artifact at brain HEAD; hotreload satisfies (1) but bypasses (2). Strict EOS-4 closure requires both halves on every surface.
...
**Steward formal assertions 2026-06-10 — FOUR surfaces now formally pass all four cycles' invariants (three strict, one substantive-pending-managed-package):**
...
| D17 | **iris portal feature-parity gap: Athena chatbot + Plutus event stream are NOT cluster-specific.** iris portal's in-portal Athena chatbot AND its real-time Plutus event stream currently route to a default backend with no cluster picker awareness. To bring iris portal to the same EOS-1/2/3/4 feature-parity as omens / turtleshell-web / turtleshell-ios — where the same user can pick api-int vs eos-4 and have their workload + accounting actually route to that cluster — iris portal needs cluster-awareness across (a) its in-portal Athena chatbot, (b) Plutus event-stream subscriber URL, (c) any other cluster-touching component (cluster details, ledger views, agent invocation panels, etc.). **Steward 2026-06-10 verbatim (initial framing as deferred):** *"iris-portal has a plutus application that streams real time data. this will need to be made cluster specific... i do not want to fix now... we may do it... but not until we have validated what is already in prod on this last go around."* **Steward 2026-06-10 reclassification (in-flight assignment):** *"nevermind i started the iris agent to bring iris-portal up to parity across the entire application for eos-1 through eos-4 which includes plutus transaction stream and the integrated turtleshell chat components."* | Observation during the 2026-06-10 multi-surface validation arc — iris portal carries the admin-write capabilities (signup + cluster spawn + AdminResponse write) but lacked the consumer-side cluster-specific feature parity that omens/ts-web/ts-ios demonstrated. | **ASSIGNED 2026-06-10 to the iris agent. In-flight during the current EOS-3+EOS-4 cycle pair.** Was briefly DEFERRED; Steward reclassified to in-flight within minutes of the original deferral. | **iris agent (olympus-616/iris repo), in-flight 2026-06-10.** Strategy per Steward direction: integrate the existing turtleshell chat components (which already carry the post-D16 cluster-picker pattern) into iris portal rather than build a parallel chat UI — code-reuse leverages the proven ts-web cluster routing. Plutus transaction stream: wire the subscriber URL to the selected cluster. | **Second cross-agent deviation cycle in this session** (D16 was first — assigned to turtleshell agent, resolved 14 min later). Pattern: deviation surfaced during validation → assigned to the owning agent → fix lands → ground-truth verifies. If iris agent ships within session, iris portal §2.8 upgrades from ✅ substantive to ✅ complete with full feature-parity across all four cycles. When iris agent fix lands, re-verify by: (1) iris-portal user picks cluster → Athena chat lands on that cluster's Pantheon (CloudWatch + ledger attribution); (2) Plutus event stream displays cluster-specific events; (3) cluster details / ledger views are scoped to the selected cluster. |
