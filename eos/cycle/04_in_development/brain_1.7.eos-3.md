...
| §2.3 omens (AppKey `guardians`) | ✅ validated against scratch (2× round-trip, full §1.2 flow) **AND** sign-in + live-refresh cluster picker validated against PRODUCTION 2026-06-10 | **Against scratch:** FB-00020 2026-06-10 02:49:38 UTC · `session_20260610_024329.jsonl` (981 KB); FB-00038 2026-06-10 04:43:31 UTC · `session_20260610_043921.jsonl` (1.05 MB) — both via `homer@cloudpremise.com` against `innovation-app-8526`. **Against production:** Steward signed into omens iPhone client as `homer@cloudpremise.com / PRODUCTION` 2026-06-10 — cluster picker initially rendered the live alpha-org cluster list with two choices (`eos-4` Provisioning + `api-int` Live `https://api-int.turtleshell.ai`). After the eos-4 CDK provision finished on the AWS side (Steward CLI), the Steward clicked **Refresh** in omens and the picker correctly re-pulled the list: `eos-4` flipped to Live with endpoint `https://api-eos-4.turtleshell.ai`. **This proves cosmos-logos discovery is real-time, not cached** — omens reads Cluster__c state from the spawning node on demand. Same row-set as iris admin UI; cross-surface consistency confirmed. Full play-cyclops-and-submit-feedback round-trip against prod still in flight. |
...
**Cumulative significance — three surfaces now demonstrate four-cycle attestation against production (in flight or complete):**
...
| **iris portal** | ✅ substantive (signup + admin UI + cluster spawn); final feedback round-trip TBD | iris signup + spawned CL-00005 `eos-4` cluster |
| **omens (iPhone)** | ✅ substantive (sign-in + cluster picker + live refresh); cyclops play + feedback submit TBD | Cluster picker rendering live alpha-org Cluster__c rows + correct refresh after eos-4 Live |

The shared `Cluster__c` row-set across iris-admin / turtleshell-web / omens is the strongest cross-surface consistency evidence the platform has produced to date.
