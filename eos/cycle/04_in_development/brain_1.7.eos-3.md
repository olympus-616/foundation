...
| D16 | **turtleshell-web cluster selector is COSMETIC for chat — picking `eos-4` in the UI does not change the API endpoint the chat POST hits.** Steward visually confirmed 2026-06-10 via browser DevTools Network tab: UI shows `CLUSTER eos-4` selected (top-right of `turtleshell.ai/app/chat`); the actual `POST /v1/athena/chat` request shows `:authority: api-int.turtleshell.ai` (NOT `api-eos-4.turtleshell.ai`). Ground-truth from ledger + CloudWatch + browser network tab agree: (a) all `llm.turn × athena` rows in 15:33–15:35 window have `ClusterName__c='int'`, zero on `eos-4`; (b) CloudWatch `/olympus/eos-4/pantheon` shows only Pulse heartbeats + health probes — no `/v1/athena/chat` requests; (c) CloudWatch `/olympus/int/pantheon` shows the actual chat POST from internal ALB IP `::ffff:10.0.0.85` with full Ares → Hermes → Athena correlation (requestIds `dd818a6d` → `2b1e4c92` → `d1174111`); (d) browser X-Request-Id `8eade4f4-af13-4249-bf70-e431616157c6` matches the request that went to `api-int.turtleshell.ai`. **omens routes correctly when its picker selects a cluster (FB-00047's `llm.turn × thoth × eos-4` at 15:08:51 is real); turtleshell-web does not.** Steward verbatim 2026-06-10: *"no it is not wokring correctly. the turtleshell-web is using the api-int regardless of the cluster selected."* | Cross-reference of Plutus ledger + olympus-int + olympus-eos-4 CloudWatch logs + browser DevTools Network tab, 2026-06-10 ~15:35 UTC. | **HIGH for EOS-4 §1.2 (user-sovereignty: pick your cluster, pay for your cluster) and HIGH for EOS-5 (single-token-economy attribution assumes ClusterName__c is truthful)** — closure-blocking if user-sovereignty per-cluster-payment is to be a real platform property. | **ASSIGNED 2026-06-10 to the turtleshell agent (cosmos-logos org, `cosmicturtle.sh`)** — the fix lives in the `cosmos-logos/turtleshell-web` repo. Steward verbatim: *"i am assigning this to the turtleshell agent to fix."* The cluster picker's `onChange` needs to swap the `apiBase` used by the chat client (and the cosmos-logos handshake URL) to the selected cluster's endpoint (`api-eos-4.turtleshell.ai` vs `api-int.turtleshell.ai`). | Steward UX perception was "no issues" because the chat ran correctly — but it ran on the wrong cluster from the user-sovereignty perspective. The omens client's `IdentityNodes.cs` is the reference for how this should work (`Prod.Node` + per-cluster routing); the ts-web equivalent appears to handle node-picking but not cluster-picking. **EOS-agent jurisdiction note:** the fix is out-of-scope for the EOS agent (cosmos-logos repo); EOS agent tracks the deviation + the fix status without authoring the code. |
