...
| D17 | **iris portal feature-parity gap: Athena chatbot + Plutus event stream are NOT cluster-specific.** iris portal's in-portal Athena chatbot AND its real-time Plutus event stream currently route to a default backend with no cluster picker awareness. To bring iris portal to the same EOS-1/2/3/4 feature-parity as omens / turtleshell-web / turtleshell-ios — where the same user can pick api-int vs eos-4 and have their workload + accounting actually route to that cluster — iris portal needs (a) a cluster selector on its Athena chatbot interface, (b) cluster-specific Plutus event-stream routing, (c) any other location in iris portal that runs against a cluster (cluster status pages, agent invocation panels, etc.) to be made cluster-aware. **Steward direction 2026-06-10 verbatim:** *"iris-portal has a plutus application that streams real time data. this will need to be made cluster specific among any other locations in the iris-portal that will need a cluster... most importantly the athena chatbot and the plutus event stream. i do not want to fix now but i want it logged and passed forward and future work that needs to be done to bring iris portal up to eos-1,eos-2,eos-3,eos-4 functionality. we may do it... but not until we have validated what is already in prod on this last go around."* | Observation during the 2026-06-10 multi-surface validation arc — iris portal carries the admin-write capabilities (signup + cluster spawn + AdminResponse write) but lacks the consumer-side cluster-specific feature parity that omens/ts-web/ts-ios have demonstrated. | **DEFERRED — NOT gating EOS-3 §2.8 iris portal closure (which is substantively ✅ via signup + admin Clusters UI + cluster spawn + cross-surface AdminResponse on FB-00046). NOT gating EOS-4 closure. NOT to be addressed in this cycle pair per Steward direction.** | **Future cycle — TBD. Candidates: EOS-5 (broader system-wide work) or a dedicated iris-portal feature-parity cycle.** Steward verbatim: *"we may do it... but not until we have validated what is already in prod on this last go around."* — validation of currently-shipped iris-portal functionality takes precedence over feature-parity work. | This is a known iris-portal-completeness gap, explicitly carried forward. Future agents picking up this work should: (1) port the omens `IdentityNodes.cs` / ts-web post-D16 cluster-picker pattern into iris portal's React/LWC code; (2) wire the in-portal Athena chatbot's `apiBase` to the selected cluster's endpoint; (3) wire the Plutus event-stream subscriber URL to the selected cluster; (4) audit iris portal for any other cluster-touching component (cluster details, ledger views, agent invocation, etc.) and apply the same pattern. **Importantly: this is NOT a bug; iris portal's current scope (admin/operator) doesn't require it. It becomes load-bearing only when iris portal is also positioned as a consumer-grade chat surface — which is a Steward product decision, not a structural requirement.** |
