...
| §2.8 iris portal | ⚠️ **partial — admin-side ✅, consumer-side feedback round-trip pending the iris-agent D17 fix (turtleshell-experience integration).** Reclassified 2026-06-10 from "✅ substantive" after Steward sharpening that iris portal had NO consumer-side feedback mechanism. | **Admin-side ✅:** (a) Signup at `app.olympus-grid.com` for `homer@cloudpremise.com`. (b) Admin Clusters page lists `api-int` (CL-00004) + spawned CL-00005 `eos-4` cluster (Pantheon image `git-2fec78e6` brain HEAD, stack `olympus-eos-4`, endpoint `api-eos-4.turtleshell.ai`, full Wizard-of-Oz provision via `zeus/scripts/cluster.sh`). (c) Cross-surface admin-WRITE: AdminResponse on FB-00046 written from iris portal. (d) D8 admin-UI visibility gap resolved. **Consumer-side ❌ (D17 expanded):** iris portal had no feedback submit affordance — meaning no iris-portal user could complete the EOS-1 round-trip from within iris portal. Strict EOS-1 compliance for iris portal pending the iris-agent fix (integrate turtleshell consumer experience). |
| §2.9 Repeatability | gates on §2.4-§2.8 | — |

**Tally as of 2026-06-10 (sixth update, post-EOS-1-sharpening): 6 of 9 criteria validated.** Demoted from 7 of 9: §2.8 iris portal reclassified from ✅ substantive to ⚠️ partial after Steward sharpened the EOS-1 definition to require consumer-side feedback round-trip — iris portal had no consumer-feedback mechanism, only admin-write. **Four surfaces still formally pass all four cycles against PRODUCTION end-to-end** — omens iPhone + turtleshell-web (post D16 fix) + turtleshell-ios (out of the box) + olympus-gpt (out of the box, hotreload with strict-EOS-4 pending next managed package). Closure-on-brain requires §2.7 (turtleshell-offgrid) + §2.9 repeatability check + the iris-agent D17 fix landing (to complete §2.8 iris portal consumer side).
