...
| §2.3 omens (AppKey `guardians`) | ✅ validated (2× round-trip, full §1.2 flow) | **FB-00020** 2026-06-10 02:49:38 UTC · attached `session_20260610_024329.jsonl` (981 KB) · **FB-00038** 2026-06-10 04:43:31 UTC · attached `session_20260610_043921.jsonl` (1.05 MB) — both via `homer@cloudpremise.com`. **Only surface to date that exercises the full §1.2 workload anchor: prod-vs-scratch login chooser → cluster chooser → workload (cyclops level) → feedback post → receipt at base.** |
...
**One in-flight gap surfaced by this attestation:** `api-int` (CL-00004) is still at `git-fd03cbd2` while newly-spawned clusters are at `git-2fec78e6`. The brain-IS-production invariant (EOS-4 §1.1) requires existing live clusters to also be at brain HEAD. Logged as §13 §1.1-deviation D9 below.
...
| D9 | **Existing live clusters do not auto-redeploy to brain HEAD when brain advances.** As of 2026-06-10 the brain HEAD Pantheon image is `git-2fec78e6` — verified because newly-spawned cluster `eos-4` (CL-00005) is being provisioned at that SHA. But the live `api-int` cluster (CL-00004) is still at `git-fd03cbd2` (the 2026-05-31 EOS-1+EOS-2 closure SHA). The EOS-4 §1.1 brain-IS-production invariant requires existing live clusters to advance with brain, not just newly-spawned ones. | Iris portal admin view at `app.olympus-grid.com/admin/clusters` (2026-06-10) showing the two clusters at different SHAs. | **HIGH for EOS-4 closure** — the brain-IS-production invariant for `olympus-int` does not hold while the live api-int is at the prior SHA. | **EOS-4 itself** — this is exactly the §2.3 (`zeus-deploy.yml` rolls `olympus-int`) work, plus possibly a §2.13 future-merge propagation gap. | Two possible patterns: (a) `zeus-deploy.yml` runs automatically on every parent merge AND advances all live clusters' Pantheon image (which would mean it's not yet wired or didn't fire correctly); (b) live cluster redeployment is intentionally manual — Steward triggers it after the new image is validated on a fresh cluster (which is what the eos-4 cluster spawn is staging for). The §2 acceptance criteria assume (a) but the operational pattern shown today is (b) — needs Steward direction. |
